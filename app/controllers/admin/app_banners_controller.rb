module Admin
  class AppBannersController < Admin::AdminController
    ...
    def index
      if params[:q].present?
        query = "%#{params[:q]}%"
        @appBanners = AppBanner.where("title LIKE :query", query: query).order(created_at: :desc).paginate(page: params[:page], per_page: 15)
      else
        @appBanners = AppBanner.all.order("id DESC").paginate(:page => params[:page], :per_page => 15)
      end
    end
